...
BUILD = HERE / "build"
for _up in [SEARCH, *SEARCH.parents]:
    if (_up / "sublimation" / "tests" / "search_corpora.py").exists():
        sys.path.insert(0, str(_up / "sublimation" / "tests"))
        break
from search_corpora import gen_corpora      # noqa: E402  reuse exact corpora

OURBIN = BUILD / "bench_sublimation"      # the SHIPPED engine, not a scaffold
...
def build():
    ...
    # Our engine: the SHIPPED library, linked the way any consumer links it.
    # This used to compile search_research.c, a scaffold whose own header says it
    # is "not the shipped engine" -- so every "sublimation" row in this table was
    # measuring a prototype. libsublimation.a comes from the normal CMake build.
    lib = None
    for _u in [SEARCH, *SEARCH.parents]:
        cand = _u / "build" / "libsublimation.a"
        if cand.exists():
            lib = cand
            break
    if lib is None:
        warn("libsublimation.a not found -- build it first: cmake --build build --target sublimation")
    else:
        r = run(["cc", "-O2", "-march=native", c_std_flag(), "-o", str(OURBIN),
                 str(HERE / "bench_sublimation.c"), str(lib),
                 "-I", str(SEARCH.parent.parent / "src" / "include"),
                 "-lpthread", "-lm"])
        if r.returncode != 0:

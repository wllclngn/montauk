...
std::string ProcessCollector::user_from_status(int32_t pid) {
  ...
  auto txt = lsm::util::read_file_string(path); if (!txt) return {};
...
bool ProcessCollector::sample(lsm::model::ProcessSnapshot& out) {
  ...
  for (auto& name : lsm::util::list_dir("/proc")) {
    ...
    auto content_opt = lsm::util::read_file_string(stat_path); if (!content_opt) continue;
    ...
    lsm::model::ProcSample ps; ps.pid=pid; ps.ppid=ppid; ps.utime=ut; ps.stime=st; ps.total_time=total_proc; ps.rss_kb = (rssp>0 ? static_cast<uint64_t>(rssp)* (getpagesize()/1024) : 0);
